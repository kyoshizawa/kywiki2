...
| v0.2       | 2026-02-19 | インフラ構成のレビュー結果反映・構成の変更 | k_yoshizawa |
| v0.3       | 2026-02-26 | レビュー結果反映 （図にバッチ関連追加） | k_yoshizawa |
...
- Routing Engine , Gateway Service の新規構築
- AP02, AP22 稼働サーバ（cfs0x, jcn0x）の ECS化
- 移行完了後の現稼働サーバ（cfs0x, jcn0x）廃止
...
- 通信サーバ：顧客単位で任意の接続先への変更が行えること（切り替えは通信サーバ AP01, AP11 から行う）
- バッチサーバ：両環境への接続が行えること
...
本番用回線（0003, 0004）を CAFIS 本番に接続し、新環境を本番稼働させる。

- 総合確認試験を実施し、本番環境での正常動作を確認する
- 問題がなければ順次顧客を新環境に向ける
- 切替後、一定期間（1週間程度）の安定稼働を確認してから次の顧客へ進む
- 全顧客の切替と安定稼働を確認後、現行環境（cfs0x サーバ + 回線 0001, 0002）の解約・撤去を実施する

### 4.2 CARDNET

CAFIS での移行完了後に着手する。基本的な移行の流れは CAFIS と同様。
...
![移行イメージ](./img/migration_plan-4.png)

- 本番用回線（3M30809-0003, 0004）を CARDNET 本番に接続し、新環境を本番稼働させる
- 総合確認試験を実施し、本番環境での正常動作を確認する
- 問題がなければ順次顧客を新環境に向ける
- 切替後、一定期間の安定稼働を確認してから次の顧客へ進む
- 全顧客終了後、AP24 の切り替えを行う
- 全切替と安定稼働を確認後、現行環境（FEXICS + 回線 3M30809-0001, 0002）の解約・撤去を実施する
