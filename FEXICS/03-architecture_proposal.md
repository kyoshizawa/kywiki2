...
| v0.4       | 2026-02-26 | コンポーネント名称の整理 (dummy server ⇒ CAFIS/CARDNET Simulator) | 内川 |
| v0.5       | 2026-02-26 | レビュー結果反映 （バッチ関連追加、証明書処理変更） | k_yoshizawa |
...
| AP05 (CAFISバッチ処理) | 更新 | btc0x | 新規環境を通信先に含む |
...
| AP23 (CARDNETバッチ処理) | 更新 | btc0x | 新規環境を通信先に含む |
...
| CAFIS/CARDNET Simulator | 新規 | - | CAFIS/CARDNETの動作を模倣する開発用シミュレータ |
...
開発環境はカード会社ネットワークへの接続が行えないため、`CAFIS/CARDNET Simulator` アプリケーションを
通信サーバ(000x) 内に配置し、CAFIS/CARDNET 動作のエミュレートを行います。
