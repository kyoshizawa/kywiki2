...
| コンポーネント | 役割 |
|---------------|------|
| **Routing Engine** | 業務アプリからの電文を受信し、仕向け先に応じて適切な Gateway Service に振り分ける |
| **Gateway Service** | CAFIS/CARDNET との接続管理および電文変換を行う。回線ごとに配置 |
...
| ログ種別 | 内容 | 保持要件 |
|---------|------|---------|
| 運用ログ | 接続状態変化、エラー、処理ログ | - |
| 監査ログ（ジャーナル） | 電文の送受信記録 | 1年以上（PCI DSS） |

---

## 4. Gateway Service（CAFIS）
