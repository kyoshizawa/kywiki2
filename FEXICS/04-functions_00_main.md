...
設定内容に基づき、業務アプリは従来の接続方式ではなく、Routing Engine を経由した電文送信を行うように動作します。

### 5.1 AP01（端末間通信:閉域）（CAFIS系統）
...
| 項目 | 現行 | 変更後 |
|------|------|--------|
| 配置 | cfs0x（物理サーバ） | ECS（新規配置） |
| 接続先 | AP03 (TCP:7000) | Routing Engine (TCP:7000) |
| 変更内容 | - | ECS対応。接続先の設定変更（ポート番号維持のため設定変更のみ） |

#### AP22（CARDNET業務サービス）

| 項目 | 現行 | 変更後 |
|------|------|--------|
| 配置 | jcn0x（物理サーバ） | ECS（新規配置） |
| 接続先 | FEXICS (DLL直接呼出し) | Routing Engine (TCP:5000) |
| 変更内容 | FexicsAPI_CN パッケージの利用 | ECS対応。TCP クライアントへの置き換え |

### 5.2 通信アプリ側の変更

移行期間中、顧客（営業所）単位で段階的に新環境へ切り替えるため、
通信アプリが顧客設定に応じて接続先（旧環境 / 新環境）を選択できるようにする。


#### AP21（Transit端末間通信）

AP01 / AP11 の CARDNET 側相当。

| 項目 | 現行 | 変更後 |
|------|------|--------|
| 配置 | ECS | ECS（変更なし） |
| 接続先 | jcn0x（固定） | 顧客設定に応じて jcn0x または Routing Engine を選択 |
| 変更内容 | 固定接続 | 顧客・端末単位での接続先切替設定を追加 |

### 5.3 AP24（Webアプリ向けAPI）

| 項目 | 現行 | 変更後 |
|------|------|--------|
| 配置 | Lambda | Lambda（変更なし） |
| 接続先 | jcn0x | Routing Engine (TCP:5000) |
| 変更内容 | 接続先の設定変更 | - |
