...
※ アプリケーションが SSL のオフロードを行う。（ALBはオフロードしない）
※ 証明書管理が必要。
...
- 電文一覧
  ...
  **カット日付**
  ...
  - Gateway Service としては、ジャーナルの過去データ削除などの契機として利用する。
...
- ジャーナルは、`シーケンス上の障害電文制御` やログとしての確認の他、過去データ整合確認などの際使用される。

  ※ その他用途： 利用情報分析

- データ量が多くなることが想定されるため、書き込みはバックグラウンド処理で一括記録を用いるなど性能面の考慮をすること。
...
- 接続ホスト名がかわるため、証明書の差し替えが必要。（クライアント側も含む）
