...
経路番号はＣＡＦＩＳセンタで取扱う電文全てに対して設定される４桁の番号です。共通制御ヘッダ部の先頭に設定され、取引種類（制御／業務）、方向（仕向／被仕向）および関連取引の引き当てに使用されます。  
自社センタにおいて取引の同時性制御が可能な経路数となります。

### 2.5.1 経路の種類

経路は制御電文の送受信に使用する「制御経路」、被仕向電文の送受信に使用する「被仕向経路」および仕向電文の送受信に使用する「仕向経路」の３種類とします。  
各経路の概要を図２．５．１－１に示します。
...
制御経路は回線数によらず２固定となります。（回線数が増えても増加なし）  
仕向経路と被仕向経路は必ず同数となります。  
１会社コードとして設定できる最大経路数は仕向経路＝被仕向経路＝１８９経路です。
...
図２．５．６－１　仕向経路開放のタイミング

（２）被仕向経路開放タイミング

被仕向経路開放タイミングを図２．５．６－２に示します。
