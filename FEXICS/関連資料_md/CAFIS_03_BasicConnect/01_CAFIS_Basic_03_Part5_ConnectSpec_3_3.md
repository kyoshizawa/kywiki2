...
## 3.3.7　ブランドデータ部(9-10-4)中継制御

### (1) 概要

ブランドデータ部(9-10-4)は、ブランド特有の情報を仕向センタ・依託向センタで送受信するためのデータ部です。

CAFISは、仕向センタ・依託向センタのフィールド利用有無に応じて、ブランドデータ部(9-10-4)を中継します。

### (2) 情報登録

仕向センタ・依託向センタはブランドデータ部(9-10-4)を利用する場合、フィールド利用有無情報の登録が必要です。

フィールド利用有無情報登録により、自センタが受信するブランド情報を、ブランド単位・Field-SubField単位で指定します。

フィールド利用有無情報の登録内容を、表3.3-8に示します。

### 表3.3-8　フィールド利用有無情報　登録内容

| 項番 | 情報名 | 内容 | 指定内容 |
|---|---|---|---|
| 1 | 処理区分 | 対象ブランドの情報登録の処理区分を選択する | 0:削除 / 1:登録(変更) / 2:登録内容の変更 / 9:全削除 |
| 2 | 処理対象 | 受信するField-SubFieldを選択する | 対象 |
| 3 | 利用区分 | Field-SubFieldを、仕向として受信するか依託向として受信するかを選択する | 1:仕向利用 / 2:依託向利用 / 3:両利用 |

（注1）ブランド単位で一括して指定可能です。\
（注2）同一ブランドで一度登録された処理区分は選択できません。\
（注3）各ブランドで利用可能なField-SubField分のみ登録が可能です。

CAFISは、中継先の「利用有無情報」に応じて中継します。中継するField-SubFieldが1つも存在しない場合、CAFISはデータ部9-10-4を削除します。

（図3.3-14　中継制御（基本）例）※図は省略

中継制御（基本）における、仕向センタ・依託向センタの処理条件を表3.3-9に、CAFISの中継仕様を表3.3-10〜表3.3-11に示します。

### 表3.3-9　仕向センタ・依託向センタ処理条件（基本）

| 区分 | 内容 | 備考 |
|---|---|---|
| 要求電文 | 以下の通り、要求電文を編集する。<br>・送信するブランド情報が存在する場合 → ブランドデータ部9-10-4の「有効」フラグを設定<br>・送信するブランド情報が存在しない場合 → ブランドデータ部9-10-4を削除 |  |
| 応答電文 | ブランドデータ部9-10-4の有無に関わらず、そのまま中継する |  |

### 表3.3-10　CAFISの中継仕様（仕向→依託）（基本）

| 区分 | 処理内容 |
|---|---|
| ブランドデータ部9-10-4「有」 | 仕向センタから受信したブランドデータ部9-10-4のうち、依託向センタのフィールド利用有無情報に基づき中継するField-SubFieldを選択し中継する。 |
| ブランドデータ部9-10-4「無」 | ブランドデータ部9-10-4を削除して中継する。 |

### 表3.3-11　CAFISの中継仕様（依託→仕向）（基本）

| 区分 | 処理内容 |
|---|---|
| ブランドデータ部9-10-4「有」 | 依託向センタから受信したブランドデータ部9-10-4のうち、仕向センタのフィールド利用有無情報に基づき中継するField-SubFieldを選択し中継する。 |
| ブランドデータ部9-10-4「無」 | ブランドデータ部9-10-4を削除して中継する。 |


### (5) 拡張データ部の検索方法

「3.3.5 拡張データ部の取扱」の(3)拡張データ部の検索方法と同様に実施します。

### (6) 拡張データ部削除時の電文編集

要求時、報告時ともに、「3.3.5 拡張データ部の取扱」の(4)拡張データ部削除時の電文編集の「(1)要求電文（一般要求・取消（再）指令・取消確認（再）指令）」の①〜③と同様に実施します。
